...
//    return NO;
    switch (state)
    {
        ...
        case DRAWING_TOOLS_BOX_STATE_RAINBOW_TOOL_SELECTED:
        {
            return NO;
        }
        ...
        default:
        {
            NSLog(@"%@ : %@ Warning! Unexpected toolbox state: %d", self, NSStringFromSelector(_cmd), state);
            return NO;
        }
    }
...
- (BOOL)allToolsAppeared 
{
    switch (activeToolType) 
    {
        ...
        {
            return rainbowTool.state == RAINBOW_TOOL_STATE_ON_SCREEN;
        }
...
- (void)saveState
{
    if(state == DRAWING_TOOLS_BOX_STATE_SELECT_DRAWING_TOOL || state == DRAWING_TOOLS_BOX_STATE_SELECT_EXTENDED_DRAWING_TOOLS ||
       state == DRAWING_TOOLS_BOX_STATE_RAINBOW_TOOL_SELECTED)
    {
        //NSLog(@"%@ : %@", self, NSStringFromSelector(_cmd));
        //NSLog(@"saving when toolbox in state %d", state);
...
- (void)restoreStateAndShowTool:(drawingToolType)tooltype
{
    ...
    //NSLog(@"restored toolbox state %d", nextState);
    ...
    switch (tooltype) 
    {
        case DRAWING_TOOL_FLOWMASTER:
        {
            ...
            if(self.drawingTools.count > 0)
            {
                ...
                for (NSUInteger i = 0;i < drawingTools.count;i++)
                {
                    ...
                    //NSLog(@"restored state %d for tool #%d", dtstate, i);
                    ...
                    for (NSUInteger j = 0;j < dt.drawingToolsExtended.count;j++)
                    {
                        ...
                        //NSLog(@"restored state %d for extended tool #%d for tool #%d", dteState, j, i);
        ...
        case DRAWING_TOOL_RAINBOW:
        {
            ...
            if(nextState == -1)
                nextState = DRAWING_TOOLS_BOX_STATE_RAINBOW_TOOL_SELECTED; //дефолтное значение
...
//    NSLog(@"restored activeToolType %d", activeToolType);
//    NSLog(@"restored nextToolType %d", nextToolType);
